<x-layouts.customer title="{{ $license->product_name }} License">
    ...
    @if($license->status === 'expired')
        <div style="background:rgba(239,68,68,0.06);border:1px solid rgba(239,68,68,0.2);border-radius:10px;padding:1rem 1.25rem;margin-bottom:1.5rem;display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap">
            <div>
                ...
                <p style="color:var(--text-muted);font-size:0.8rem;margin:0.25rem 0 0;">This license expired on {{ $license->end_date?->format('d M Y') }}. Submit a renewal request to restore access.</p>
            </div>
            <a href="{{ route('customer.tickets.create', ['locale' => app()->getLocale()]) }}?subject=License+Renewal+Request&type=billing"
    ...
    <div class="cp-section-card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;">
            <div>
                <p style="color:var(--text-muted);font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">License Key</p>
                <p style="color:#00C896;font-family:monospace;font-size:0.9rem;word-break:break-all;">{{ $license->license_key }}</p>
            </div>
            <div>
                <p style="color:var(--text-muted);font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">Status</p>
                <p style="color:{{ $statusColor }};font-size:0.9rem;font-weight:600;text-transform:capitalize;">{{ $license->status }}</p>
            </div>
            <div>
                <p style="color:var(--text-muted);font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">Product</p>
                <p style="color:#e2e8f0;font-size:0.875rem;">{{ $license->product_name }}</p>
            </div>
            <div>
                <p style="color:var(--text-muted);font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">Plan</p>
                <p style="color:#e2e8f0;font-size:0.875rem;">{{ \App\Models\License::planLabel($license->plan) }}</p>
            </div>
            <div>
                <p style="color:var(--text-muted);font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">Authorized Seats</p>
                <p style="color:#e2e8f0;font-size:0.875rem;">{{ $license->seats }}</p>
            </div>
            <div>
                <p style="color:var(--text-muted);font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">Valid From</p>
                <p style="color:#e2e8f0;font-size:0.875rem;">{{ $license->start_date?->format('d M Y') }}</p>
            </div>
            <div>
                <p style="color:var(--text-muted);font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">Expires</p>
                <p style="color:{{ $expiring ? '#eab308' : '#e2e8f0' }};font-size:0.875rem;">{{ $license->end_date?->format('d M Y') }}</p>
            </div>
            @if($license->price)
            <div>
                <p style="color:var(--text-muted);font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">License Fee</p>
        ...
        <div style="margin-top:1.5rem;padding-top:1.5rem;border-top:1px solid #334155;">
            ...
            <p style="color:var(--text-muted);font-size:0.875rem;line-height:1.6;">{{ $license->notes }}</p>
        </div>
        @endif
    </div>

    <div class="cp-section-card" style="margin-top:1rem;">
        <p style="color:var(--text-muted);font-size:0.8125rem;line-height:1.7;">
