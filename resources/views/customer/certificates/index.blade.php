<x-layouts.customer title="Certificates">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900">My Certificates</h1>
        <p class="text-slate-500 mt-1">Download certificates for courses you have completed.</p>
    </div>
            ...
            @foreach($certificates as $cert)
                <div class="bg-white rounded-xl border border-slate-200 p-5 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <i data-lucide="award" style="width:28px;height:28px" class="text-emerald-500"></i>
                        <div>
                            <h3 class="font-semibold text-slate-800">{{ $cert->course?->title ?? 'Course' }}</h3>
